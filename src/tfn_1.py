...
class MULTModel(nn.Module):
    def __init__(self, hyp_params):
        ...
        #CNN
        self.cnn_ls = nn.Conv1d(2*self.d_l, 10, kernel_size=1, padding=0, bias=False)
        self.cnn_as = nn.Conv1d(2*self.d_a, 10, kernel_size=1, padding=0, bias=False)
        self.cnn_vs = nn.Conv1d(2*self.d_v, 10, kernel_size=1, padding=0, bias=False)
        self.cnn_lstm = nn.LSTM(input_size = 30, hidden_size=10, batch_first=True, bidirectional=False)
    ...
    def forward(self, x_l, x_a, x_v):
        ...
        tmp_x_lav = F.dropout(tmp_x_lav, 0.5)
        ...
        if self.lonly:
            ...
            tmp_ls = h_ls.permute(1, 2, 0)
            tmp_ls = self.cnn_ls(tmp_ls)
            F.dropout(tmp_ls, 0.2)
        ...
        if self.aonly:
            ...
            tmp_as = h_as.permute(1, 2, 0)
            tmp_as = self.cnn_as(tmp_as)
            F.dropout(tmp_as, 0.3)
        ...
        if self.vonly:
            ...
            tmp_vs = h_vs.permute(1, 2, 0)
            tmp_vs = self.cnn_vs(tmp_vs)
            F.dropout(tmp_vs, 0.3)
        ...
        if self.partial_mode == 3:
            ...
            # last_hs 24 180
        tmp_lav = torch.cat([tmp_ls,tmp_as,tmp_vs], dim=1).permute(0, 2, 1)
        tmp_lav,_ = self.cnn_lstm(tmp_lav)
        F.dropout(tmp_lav, 0.3)
        last_hs = torch.cat([last_hs, tmp_lav[:,-1,:], tfn_lav], dim=1)
